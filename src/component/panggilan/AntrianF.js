...
function Antrian() {
    ...
    const setStore = async (no_reg, kd_dokter, kd_poli, no_rawat, no_rkm_medis, nm_pasien, nm_poli, nm_dokter, tgl_registrasi, kelas) => {
        if (disable.length === 0) {
            ...
            setDisable(kelas)
            try {
                const result = await storeData("storef", {
                    id: count,
                    kd_dokter: kd_dokter,
                    kd_poli: kd_poli,
                    status: 1,
    ...
    const setUpdate = async (tgl_registrasi, no_rkm_medis, kd_dokter, kd_poli, nm_pasien, nm_poli, nm_dokter, no_reg) => {
        setDisable([]);
        try {
            const result = await updateData("updatef", tgl_registrasi, no_rkm_medis, kd_dokter, kd_poli, {
                status: 2,
                nm_pasien: nm_pasien,
                nm_dokter: nm_dokter,
                nm_poli: nm_poli,
    ...
    return (
        <AntrianPoli
            ...
            reload={reload}
            title="Panggil Antrian Poliklinik F"
            headerClass="header-antrian-f"
        />
    );
}
